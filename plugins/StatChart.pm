...
my @graph_presets = (
    {   id       => 'graph-loans-dow',
        ...
        type     => "bar",
        query =>
            "SELECT DAYOFWEEK(issuedate), DAYNAME(issuedate), COUNT(issuedate) FROM (SELECT issuedate FROM issues UNION SELECT issuedate FROM old_issues) AS all_issues GROUP BY DAYOFWEEK(issuedate);"
    },
    ...
    {   id       => 'graph-loans-year',
        ...
        type     => "bar",
        query =>
            "SELECT YEAR(issuedate), YEAR(issuedate), COUNT(*) FROM (SELECT issuedate FROM issues UNION SELECT issuedate FROM old_issues) AS all_issues GROUP BY YEAR(issuedate) ORDER BY YEAR(issuedate) ASC;"
    }
);

my $locale = "en_US";
my @supportedLocales = ( 'en', 'fr' );
...
sub home {
    ...
    $template->param( locale => loc_code(), graph_presets => \@graph_presets );
...
sub graph {
    ...
    for my $key ( $cgi->param('graphs') ) {
        for my $preset (@graph_presets) {
            if ( $key eq $preset->{id} ) {
                push @graphs, { type => $preset->{type}, title => $preset->{ 'title_' . loc_code() }, units => $preset->{ "units_" . loc_code() }, data => fetch( $preset->{query} ) };
    ...
    $template->param( locale => loc_code(), graphs => \@graphs );
...
sub weekday_fixer {
    ...
    for my $i ( 0 .. 6 ) {
        ...
        # Add missing keys
        if ( $list[$i]{key} != $i + 1 ) {
            splice @list, $i, 0, { key => $i + 1, name => $dayname{ loc_code() }[$i], count => 0 };
        }

        # Translate existing keys
        else {
            $list[$i]{name} = $dayname{ loc_code() }[$i];
        }
    }
    return \@list;
}
...
sub loc_code {
    my $str = substr( $locale, 0, 2 );

    for my $loc (@supportedLocales) {
        if ( $loc eq $str ) {
            return $str;
        }
    }
